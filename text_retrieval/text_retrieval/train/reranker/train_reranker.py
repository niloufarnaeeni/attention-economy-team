...
from rag_retrieval.train.reranker.data import (
    ...
    GroupedRankerDatasetWithYaps,
)
from rag_retrieval.train.reranker.trainer import Trainer
...
def save_run_config(run_dir: Path, cfg: dict, run_id: str) -> None:
    cfg = dict(cfg)
    cfg["run_time"] = run_id  # SAME STRING, SAME FORMAT

    out_path = run_dir / "model" / "run_config.json"
...
def main():
    ...
    # Redirect all outputs to the timestamp folder
    ...
    if "pointwise" == args.train_dataset_type:
    ...
    elif "grouped" == args.train_dataset_type and args.loss_type in {"pairwise_ranknet_yap_neg", "pairwise_ranknet_yap_both", "pairwise_ranknet_yap_pos"}:
        train_dataset = GroupedRankerDatasetWithYaps(
            data_path=args.train_dataset,
            label_key=args.train_label_key,
            target_model=model,
            max_len=args.max_len,
        ...
        model.lambda_ = float(getattr(args, "lambda", 0.3))
    ...
    if args.loss_type == "pairwise_ranknet_centered":
        model.lambda_ = float(getattr(args, "lambda", 0.2))
    ...
    if args.val_dataset:
        ...
        elif "grouped" == args.val_dataset_type and args.loss_type in {"pairwise_ranknet_yap_neg","pairwise_ranknet_yap_both", "pairwise_ranknet_yap_pos"}:
            val_dataset = GroupedRankerDatasetWithYaps(
                data_path=args.val_dataset,
                label_key=args.val_label_key,
                target_model=model,
                max_len=args.max_len,
    ...
    if accelerator.is_main_process:
        save_run_config(run_dir, vars(args), run_id)
