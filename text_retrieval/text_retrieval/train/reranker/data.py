...
from rag_retrieval.train.reranker.utils import (
    map_label_to_continuous,
...
class GroupedRankerDatasetWithYaps(GroupedRankerDataset):
    """
    Extends GroupedRankerDataset by adding yap_score support.
    ...
    def read_data(self, data_path):
        ...
        with open(data_path, "r", encoding="utf-8") as f:
            for line in tqdm.tqdm(f, desc=f"Loading {self.tag}+YAPS"):
                row = json.loads(line)

                query = row["query"].strip()
                hits = row["hits"]

                if len(hits) < self.train_group_size:
                    continue

                random.shuffle(hits)

                for i in range(0, len(hits), self.train_group_size):
                    group = hits[i:i + self.train_group_size]
                    ...
                    for h in group:
                        ...
                        docs.append(text)
                        labels.append(float(h.get(self.label_key, 0.0)))
                        yaps.append(float(h.get("yap_score", 0.0)))

                    # ---- padding (same logic as base class) ----
                    if len(docs) < self.train_group_size:
                        candidate_range = len(hits)
                        add_idx = random.sample(
                            range(candidate_range),
                            self.train_group_size - len(docs),
                        )
                        for idx in add_idx:
                            h = hits[idx]
                            docs.append(h["content"].strip())
                            labels.append(float(h.get(self.label_key, 0.0)))
                            yaps.append(float(h.get("yap_score", 0.0)))
                    ...
                    data.append([query, docs, labels, yaps])

        if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
            print(f"Loaded {len(data)} grouped samples with YAPS")

        return data

    def collate_fn(self, batch):
        all_pairs, all_labels, all_yaps = [], [], []

        for query, docs, labels, yaps in batch:
            for d, l, y in zip(docs, labels, yaps):
                all_pairs.append([query, d])
                all_labels.append(l)
                all_yaps.append(y)

        tokens = self.model.preprocess(all_pairs, self.max_len)

        return tokens, {
            "labels": torch.tensor(all_labels, dtype=torch.float32),
            "yaps": torch.tensor(all_yaps, dtype=torch.float32),
        }
